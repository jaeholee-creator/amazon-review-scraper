"""
Amazon Review Scraper - API Mode v2
...
Usage:
    python api_daily_scraper.py              # 전체 제품 실행
    python api_daily_scraper.py --test       # 테스트 모드 (10페이지)
    python api_daily_scraper.py --limit 3    # 3개 제품만
"""
...
API_URL = 'https://www.amazon.com/portal/customer-reviews/ajax/reviews/get/ref=cm_cr_arp_d_paging_btm_next_1'
...
async def main():
    ...
    print(f"\n{'='*60}")
    print("   BIODANCE Daily Review Scraper (API v2)")
    print(f"{'='*60}")
    ...
    session = BrowserSession(region='us')  # US 설정 사용
    ...
    results = []
    output_file = f'data/daily/{run_date}/all_reviews_api.csv'
    file_started = False
